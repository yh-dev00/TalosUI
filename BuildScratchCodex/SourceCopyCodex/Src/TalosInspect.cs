...
namespace TalosCore
{
    ...
    public class UiAutomationService : IUiAutomationService
    {
        ...
        private const int NativeUiAutomationProcessIdPropertyId = 30002;
        private const int NativeUiAutomationNativeWindowHandlePropertyId = 30020;
        private const int MaxHitTestRefinementDepth = 32;
        ...
        private AutomationElement GetAutomationElementFromPhysicalPoint(double physicalX, double physicalY)
        {
            ...
            AutomationElement element = AutomationElement.FromPoint(new WindowsPoint(physicalX, physicalY));
            ...
            return RefineVirtualizedHitTestElement(element, physicalX, physicalY);
        }

        private AutomationElement RefineVirtualizedHitTestElement(AutomationElement element, double physicalX, double physicalY)
        {
            if (element == null)
            {
                return null;
            ...
            if (TryIsWindowlessElement(current, out isWindowless) && isWindowless)
            {
                WindowsRect currentRectangle;

                if (!TryGetPhysicalBoundingRectangle(current, out currentRectangle) ||
                    !RectangleContainsPoint(currentRectangle, physicalX, physicalY))
                {
                    AutomationElement containingAncestor = FindNearestContainingAncestor(current, physicalX, physicalY);

                    if (containingAncestor != null)
                    {
                        current = containingAncestor;
                    }
                }
            }

            return RefineToSmallestContainingDescendant(current, physicalX, physicalY);
        }

        private AutomationElement FindNearestContainingAncestor(AutomationElement element, double physicalX, double physicalY)
        {
            TreeWalker walker = TreeWalker.ControlViewWalker;
            AutomationElement current = element;

            for (int depth = 0; depth < MaxHitTestRefinementDepth; depth++)
            {
                AutomationElement parent;

                if (!TryGetControlViewParent(walker, current, out parent) ||
                    parent == null ||
                    AutomationElement.RootElement.Equals(parent))
                {
                    return null;
                }

                WindowsRect parentRectangle;

                if (TryGetPhysicalBoundingRectangle(parent, out parentRectangle) &&
                    RectangleContainsPoint(parentRectangle, physicalX, physicalY))
                {
                    return parent;
                }
        ...
        private AutomationElement RefineToSmallestContainingDescendant(AutomationElement element, double physicalX, double physicalY)
        {
            AutomationElement current = element;

            for (int depth = 0; depth < MaxHitTestRefinementDepth; depth++)
            {
                AutomationElement child = FindSmallestContainingChild(current, physicalX, physicalY);
        ...
        private AutomationElement FindSmallestContainingChild(AutomationElement element, double physicalX, double physicalY)
        {
            AutomationElementCollection children;

            if (!TryFindControlViewChildren(element, out children))
            {
                return null;
            }
            ...
            foreach (AutomationElement child in children)
            {
                WindowsRect childRectangle;

                if (!TryGetPhysicalBoundingRectangle(child, out childRectangle) ||
                    !RectangleContainsPoint(childRectangle, physicalX, physicalY))
                {
                    continue;
                }

                double area = GetRectangleArea(childRectangle);
        ...
        private bool TryIsWindowlessElement(AutomationElement element, out bool isWindowless)
        {
            isWindowless = false;

            try
            {
                isWindowless = GetIntProperty(element, AutomationElement.NativeWindowHandleProperty) == 0;
                return true;
            }
            catch (ElementNotAvailableException)
            {
        ...
        private bool TryGetControlViewParent(TreeWalker walker, AutomationElement element, out AutomationElement parent)
        {
            parent = null;

            if (walker == null || element == null)
            {
                return false;
            }
        ...
        private bool TryFindControlViewChildren(AutomationElement element, out AutomationElementCollection children)
        {
            children = null;
        ...
        private bool TryGetPhysicalBoundingRectangle(AutomationElement element, out WindowsRect rectangle)
        {
            rectangle = WindowsRect.Empty;

            try
            {
                rectangle = GetPhysicalBoundingRectangle(element);
        ...
        private bool RectangleContainsPoint(WindowsRect rectangle, double physicalX, double physicalY)
        {
            ...
            return physicalX >= rectangle.X &&
                physicalX <= rectangle.X + rectangle.Width &&
                physicalY >= rectangle.Y &&
                physicalY <= rectangle.Y + rectangle.Height;
        }

        private double GetRectangleArea(WindowsRect rectangle)
        {
            if (rectangle.IsEmpty)
            {
        ...
        private string GetInspectName(AutomationElement element, int? screenX, int? screenY)
        {
            ...
            if (screenX.HasValue && screenY.HasValue &&
                TryGetNativeNameFromPoint(element, screenX.Value, screenY.Value, out nativeName))
            {
        ...
        private bool TryGetNativeNameFromPoint(AutomationElement resolvedElement, int screenX, int screenY, out string name)
        {
            name = string.Empty;
            ...
            try
            {
                ...
                if (!NativeElementMatchesResolvedElement(nativeElement, resolvedElement))
                {
                    return false;
                }
        ...
        private bool NativeElementMatchesResolvedElement(IUIAutomationElement nativeElement, AutomationElement resolvedElement)
        {
            if (nativeElement == null || resolvedElement == null)
            {
                return false;
            }

            int nativeProcessId = GetNativeIntProperty(nativeElement, NativeUiAutomationProcessIdPropertyId);
            int resolvedProcessId = GetIntProperty(resolvedElement, AutomationElement.ProcessIdProperty);

            if (nativeProcessId > 0 && resolvedProcessId > 0 && nativeProcessId == resolvedProcessId)
            {
                return true;
            }

            IntPtr nativeWindowHandle = GetNativeWindowHandle(nativeElement);
            IntPtr resolvedWindowHandle = GetContainingNativeWindowHandle(resolvedElement);

            if (nativeWindowHandle == IntPtr.Zero || resolvedWindowHandle == IntPtr.Zero)
            {
                return false;
            }

            return GetRootWindowHandle(nativeWindowHandle).Equals(GetRootWindowHandle(resolvedWindowHandle));
        }

        private IntPtr GetNativeWindowHandle(IUIAutomationElement nativeElement)
        {
            int nativeWindowHandle = GetNativeIntProperty(nativeElement, NativeUiAutomationNativeWindowHandlePropertyId);
            return nativeWindowHandle == 0 ? IntPtr.Zero : new IntPtr(nativeWindowHandle);
        }

        private int GetNativeIntProperty(IUIAutomationElement nativeElement, int propertyId)
        {
            ...
            object value = nativeElement.GetCurrentPropertyValueEx(propertyId, true);
